...
app.post("/api/register", async (req, res) => {
  try {
    ...
    const user = new User({ username: email, email, password, authMethod });
    await user.save();
...
app.get("/api/register", authenticateToken, async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email: req.user.email }, { password: 0 }); // Exclude the password field from the response
    if (!user) {
